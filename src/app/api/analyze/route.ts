...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[API Proxy] Received request:', JSON.stringify(body, null, 2));
    
    // Forward the request to the external API
    const response = await fetch(`${API_BASE_URL}/analysis/advanced`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to call analysis API' },
      { status: 500 }
    );
  }
}
